...
# nested middleware, error handlers and route caching.
# 
# A RoutingTree instance takes the given directory (root_dir) and constructs a
# tree of route entries corresponding to the directory's contents. Finally, an
# optimized router proc is generated, which returns a route entry for a given
# path.
# 
...
#   `/docs/_bar.rb` are skipped and will not be routed. This allows you to
#   prevent access through the HTTP server to protected or internal modules or
#   files.
class RoutingTree
  ...
  def visit_routing_tree_entry(buffer:, entry:, indent: 1, segment_idx:)
    ...
    if entry[:children]
      entry[:children].each do |k, child_entry|
        # skip if wildcard entry, or entry has no target or no children
        next if k == '[]'

        # skip if entry is void (no target, no children)
        target = child_entry[:target]
        has_children = child_entry[:children] && !child_entry[:children].empty?
        next if !target && !has_children
