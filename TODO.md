...
## Embedded Papercraft template in Markdown

What if we wanted to embed a Papercraft template in a markdown document/article,
such that the template will be actually rendered into the page? Perhaps syntax
as follows:

```ruby
# render: true
section {
  button '0', id: 'increment-counter'

  script <<~JS
    const button = document.querySelector('increment-counter')
    let counter = 0;
    button.onclick = (e) => {
      e.preventDefault();
      counter++;
...
This could be an cool way to add some interactivity examples to articles, and
even to add some general interactivity to the website. We can also inject the usual module instance variables into a context: @app, @ref, @machine, @env.
...
At the same time, we can also improve how markdown is rendered. Currently, we
compute a controller proc, taking into account the specified layout. We can
instead wrap it in a custom module context that injects the markdown into the
module. One interesting question: do we process the markdown once, or on each
render?
...
md = md.gsub(/^```ruby\n# render: true\n(.*?)\r?\n```\n/m) {
  snippet = Regexp.last_match[1]
  templ = @templates[snippet] ||= prepare_snippet_template(snippet)
  templ.render
}

def prepare_snippet_template(snippet, location)
  instance_eval "->() do\n#{snippet}\nend"
end
```
